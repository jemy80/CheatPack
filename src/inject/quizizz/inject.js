...
const checkGame = setInterval(() => {
	if (window.location.href.search("quizizz.com/join/game/") != -1 && window.location.href.search("gameType=") != -1 && window.location.href.indexOf('cheatpack') == -1) {
			...
			chrome.runtime.sendMessage({ query: "serverAddress" }, function (response) {
				...
				xhr.onload = () => {
					if (xhr.status == 200) {
						...
						if (scriptNumber == 2) {
							console.info(`%c${chrome.i18n.getMessage('quizizzLoading')}`, "font-style: italic"); // Console log to make sure the script has been loaded.
						}
					}
				}
